...
type
  TdmDV = class(TDataModule)
    ...
    qUserUnitSuiteSampleNumFracNUMFRAC: TFMTBCDField;
    cdsUserUnitSuiteSampleNumFracNUMFRAC: TFMTBCDField;
  ...
  public
    ...
    procedure InsertUserProject(tUserProjectID : integer; tUserID, tUserProjectName : string; var WasSuccessful : boolean);
  end;
...
procedure TdmDV.InsertUserProject(tUserProjectID : integer; tUserID, tUserProjectName : string; var WasSuccessful : boolean);
var
  TD: TDBXTransaction;
begin
  UserSession.WhereAmI := 'InsertUserProject start';
  dmDV.qInsertUserProject.Close;
  dmDV.qInsertUserProject.SQL.Clear;
  dmDV.qInsertUserProject.SQL.Add('INSERT INTO USERPROJECTS');
  dmDV.qInsertUserProject.SQL.Add('(UserProjectID,UserID,UserProjectName)');
  //dmDV.qInsertUserProject.SQL.Add('Values (:UserProjectID, :UserID, :UserProjectName)');
  //dmDV.qInsertUserProject.ParamByName('UserProjectID').AsInteger := tUserProjectID;
  //dmDV.qInsertUserProject.ParamByName('UserID').AsString := tUserID;
  //dmDV.qInsertUserProject.ParamByName('UserProjectName').AsString := tUserProjectName;
  dmDV.qInsertUserProject.SQL.Add('Values ('+IntToStr(tUserprojectID)+','+QuotedStr(tUserID)+','+QuotedStr(tUserProjectName)+')');
  //dmUser.SetDeveloperData(dmDV.qInsertUserProject.SQL.Text);
  //dmUser.SetDeveloperData('UserProjectID = '+IntToStr(tUserProjectID));
  //dmUser.SetDeveloperData('UserID = '+tUserID);
  //dmUser.SetDeveloperData('UserProjectName = '+tUserProjectName);
  TD := dmDV.sqlcDateView.BeginTransaction(TDBXIsolations.ReadCommitted);
  try
    dmDV.qInsertUserProject.ExecSQL(true);
    dmDV.sqlcDateView.CommitFreeAndNil(TD); //on success, commit the changes;
    WasSuccessful := true;
  except
    dmDV.sqlcDateView.RollbackFreeAndNil(TD); //on failure, undo the changes;
